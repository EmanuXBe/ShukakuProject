...
class Rules:
    ...
    def is_valid_rectangle(
        board: Board, start_row: int, start_col: int, height: int, width: int
    ) -> bool:
        """
        ...
        # Lookup O(1) por posición para evitar el bucle anidado O(celdas × pistas).
        clue_by_pos = {(c.row, c.col): c for c in board.clues.values()}
        ...
        for r in range(start_row, start_row + height):
            for c in range(start_col, start_col + width):
                ...
                if (r, c) in clue_by_pos:
                    clues_found.append(clue_by_pos[(r, c)])
